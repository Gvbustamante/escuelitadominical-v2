  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (role === 'admin' || role === 'coordinador') {
        ...
        if (error) console.error('useMisHijos (admin):', error.message)
        ...
        if (errAsig) console.error('useMisHijos (docente asig):', errAsig.message)
        ...
        if (nivelIds.length > 0) {
          const { data, error } = await supabase
            ...
            .order('nombre_completo')
          if (error) console.error('useMisHijos (docente ninos):', error.message)
          resultado = (data || []).map((n) => ({ ...n, parentesco: null }))
        }
      } else {
        ...
        const { data, error } = await supabase
          ...
          .eq('padre_id', user.id)
        if (error) console.error('useMisHijos (padre):', error.message)
