...
| `bitacora_clase` | Constancia por clase, fecha y `momento` (`antes`/`despues`): `salon_ok` + `salon_foto_url`, `refrigerio_detalle` + `refrigerio_foto_url` (solo `despues`), `notas` (única por `nivel_id`+`fecha`+`momento`, ver 5.10) | `niveles`, `docente_id` → `profiles` |
