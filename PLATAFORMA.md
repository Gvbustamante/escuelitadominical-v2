...
- **Bitácora** (`bitacora_clase`, rutas `/bitacora`): el docente deja constancia, por clase y fecha, de si el salón quedó en buen estado (con foto) y qué refrigerio se dio (con foto) — una foto por cada cosa, no una galería. Admin/coordinador ven el historial por mes y clase en `BitacoraAdmin.jsx`, y también pueden **registrar una bitácora ellos mismos** (botón "📝 Registrar bitácora", eligiendo la clase) — la RLS de `bitacora_clase` ya lo permitía desde el inicio, solo faltaba el formulario en la pantalla de admin.
...
### 5.20 Usuarios, permisos por rol, y docentes editando niños
...
- Permisos curados (todos empiezan en el valor de la lista, editables en **Usuarios → Roles y permisos**, solo admin):
...
- **`Usuarios.jsx`** (ruta `/usuarios`, admin/coordinador): pantalla nueva que lista **todas** las cuentas de cualquier rol (admin, coordinador, docente, padre) en un solo lugar, con búsqueda y filtro por rol. "+ Nuevo usuario" crea cualquier rol (reutiliza `admin_create_invited_user`, pidiendo el niño/a cuando el rol es padre). "Ver detalle" (`DetalleUsuarioModal.jsx`) permite editar nombre/cédula/teléfono, y si quien mira es `admin` (y no es su propia cuenta) también **cambiar el rol** y **restablecer la contraseña** a la de por defecto (`admin_reset_password(user_id)`, misma regla de quién-puede-tocar-a-quién que al invitar). No reemplaza los atajos que ya existían (Docentes → +Agregar, Niños → +Padre) — los complementa. La pestaña **"Roles y permisos"** (solo visible para `admin`) vive dentro de la misma pantalla y edita `permisos_rol`.
...
- `docente/Actividades.jsx` ganó los mismos botones ✏️ Editar y 🗑️ Eliminar que ya tenía el admin (en ambas vistas, compacta y tarjetas) — la RLS de `actividades` ya lo permitía para el nivel propio, solo faltaba el botón.
...
| `tarea_entregas` | Una fila por niño **o** por docente (exactamente uno de los dos, ver 5.23) para cada actividad-tarea: `estado` (pendiente/pausada/entregada), `archivo_url`, `comentario_padre` (HTML), `nota_docente` (única por `actividad_id`+`nino_id` y por `actividad_id`+`docente_id`) | `actividades`, `ninos`, `docente_id`/`entregado_por` → `profiles` |
...
| `horarios` | Servicios que se repiten cada día de clase (ej. "9:00 am"); siempre trae al menos "Servicio único" (ver 5.18) | — |
...
- `admin_create_invited_user(...)`: ver sección 5.4. Es la única forma de crear usuarios nuevos (no hay sign-up público). Desde 5.20, también acepta a un `docente` con permiso `vincular_padres` como llamador (restringido a crear padres de sus propios niños).
- `admin_reset_password(user_id)`: ver 5.20. Regenera la contraseña de una cuenta a `cédula.`, mismas reglas de admin/coordinador que la anterior.
